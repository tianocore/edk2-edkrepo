...
class BaseTestBaseXmlHelper:
    ...
    def test_dict_with_all_optional_fields(self, mock_et_module):
        """When the dict contains name, attrib, text, tail, and children, build_etree_node must set all fields and recurse."""
        def create_subelement_mock(parent, tag, attrib=None):
            return self._create_mock_subelement(parent, tag, attrib)
    ...
    def test_dict_with_name_only(self, mock_et_module):
        """When the dict contains only the name key, build_etree_node must create a SubElement with empty attrib, None text, None tail, and no children."""
        def create_subelement_mock(parent, tag, attrib=None):
            return self._create_mock_subelement(parent, tag, attrib)
