...
class TestManifestXml:
    ...
    def test_is_pin_file_returns_true_when_xml_type_is_pin(self, manifest_instance):
        """When _xml_type is 'Pin', is_pin_file must return True."""
        manifest_instance._xml_type = ELEMENT_TAG_PIN
        assert manifest_instance.is_pin_file() is True

    def test_is_pin_file_returns_false_when_xml_type_is_manifest(self, manifest_instance):
        """When _xml_type is 'Manifest', is_pin_file must return False."""
        assert manifest_instance.is_pin_file() is False
    ...
    def test_get_parent_of_nested_repo_raises_for_non_nested_path(self, manifest_instance):
        """When repo_local_root has only one component, must raise ValueError."""
        with pytest.raises(ValueError) as exc_info:
            manifest_instance.get_parent_of_nested_repo([], INVALID_REPO_PATH)
        assert str(exc_info.value) == INVALID_REPO_PATH_ERROR.format(INVALID_REPO_PATH)
    ...
    def test_get_parent_of_nested_repo_raises_when_no_parent_found(self, manifest_instance):
        """When no source matches any parent directory, must raise ValueError."""
        with pytest.raises(ValueError) as exc_info:
            manifest_instance.get_parent_of_nested_repo([], NESTED_REPO)
        assert str(exc_info.value) == NO_PARENT_REPO_ERROR.format(NESTED_REPO)
    ...
    def test_get_submodule_alternates_returns_matching_for_remote(self, manifest_instance):
        """When a submodule alternate matches remote_name, must return a list with its tuple."""
        mock_alt = self._make_mock_alt(REMOTE_NAME)
        manifest_instance._submodule_alternate_remotes = [mock_alt]
        result = manifest_instance.get_submodule_alternates_for_remote(REMOTE_NAME)
        assert result == [mock_alt.tuple]

    def test_get_submodule_alternates_returns_empty_when_no_match(self, manifest_instance):
        """When no submodule alternate matches remote_name, must return an empty list."""
        mock_alt = self._make_mock_alt(UNKNOWN_REMOTE)
        manifest_instance._submodule_alternate_remotes = [mock_alt]
        assert manifest_instance.get_submodule_alternates_for_remote(REMOTE_NAME) == []
    ...
    def test_get_patchsets_for_combo_returns_all_when_combo_is_none(self, manifest_instance):
        """When combo is None, must scan all combos and return a dict of found patchsets."""
        ps = self._setup_combo_with_patchset(manifest_instance)
        result = manifest_instance.get_patchsets_for_combo(combo=None)
        assert COMBO_NAME in result
        assert result[COMBO_NAME] == ps

    def test_get_patchsets_for_combo_returns_matching_when_combo_specified(self, manifest_instance):
        """When a specific combo is given, must return only its patchsets."""
        ps = self._setup_combo_with_patchset(manifest_instance)
        result = manifest_instance.get_patchsets_for_combo(combo=COMBO_NAME)
        assert result[COMBO_NAME] == ps
