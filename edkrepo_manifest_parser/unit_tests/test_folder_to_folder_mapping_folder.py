...
from edkrepo_manifest_parser.manifest_parser_unit_test_helpers.helpers import (
    ...
    ATTRIB_PROJECT2,
    TAG_EXCLUDE,
    FIELD_PROJECT1_FOLDER,
    FIELD_PROJECT2_FOLDER,
    PROJECT1_FOLDER,
    PROJECT2_FOLDER,
    EXCLUDE_PATH,
    ATTRIB_FIELD_FIELDS,
...
ATTRIB_FIELD_IDS = [FIELD_PROJECT1_FOLDER, FIELD_PROJECT2_FOLDER]
...
class TestFolderToFolderMappingFolder:
    ...
    @pytest.mark.parametrize(ATTRIB_FIELD_FIELDS, [
        (ATTRIB_PROJECT1, PROJECT1_FOLDER, FIELD_PROJECT1_FOLDER),
        (ATTRIB_PROJECT2, PROJECT2_FOLDER, FIELD_PROJECT2_FOLDER),
    ], ids=ATTRIB_FIELD_IDS)
    def test_single_attrib_field_when_present(self, attrib_key, attrib_value, field_name):
        """When a single optional project attribute is present, __init__ must set the corresponding field to its value."""
    ...
    def test_excludes_populated_from_exclude_children(self):
