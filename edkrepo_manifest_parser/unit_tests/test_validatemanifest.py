...
class TestValidateManifest:
    ...
    def mock_manifest_xml(self):
        """Patch ManifestXml for the duration of a test; yields the mock class."""
        with patch('edkrepo_manifest_parser.edk_manifest_validation.ManifestXml') as mock:
            yield mock

    @pytest.fixture
    def mock_manifest_xml_instance(self, mock_manifest_xml):
        """Configure mock_manifest_xml to return a fresh MagicMock and yield that instance."""
        instance = MagicMock()
        mock_manifest_xml.return_value = instance
        yield instance
    ...
    def test_try_parse_manifest_xml_returns_xmldata_on_success(self, mock_manifest_xml_instance):
        """When ManifestXml succeeds, must return (xmldata, None) with the constructed instance."""
        self._assert_try_parse_result(mock_manifest_xml_instance, None)
    ...
    def test_validate_parsing_success(self, vm, mock_manifest_xml_instance):
        """When ManifestXml succeeds, must return ("PARSING", True, None) and set _manifest_xmldata."""
        self._assert_parsing_result(vm, (RESULT_PARSING, True, None), mock_manifest_xml_instance)
