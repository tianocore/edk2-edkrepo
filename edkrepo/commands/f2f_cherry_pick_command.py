        ...
        while todo_commits:
            ...
            if not continue_operation:
                ...
                if num_cherry_picks == 1:
                    ui_functions.print_info_msg(humble.F2F_CHERRY_PICK_NUM_CHERRY_PICKS_SINGULAR.format(len(cherry_pick_operations)))
                else:
                    ui_functions.print_info_msg(humble.F2F_CHERRY_PICK_NUM_CHERRY_PICKS_PLURAL.format(len(cherry_pick_operations)))
    ...
    for operation in cherry_pick_operations:
        ...
        for folder in operation:
            ...
            if not git_path_exists(source, src_commit, repo):
                ui_functions.print_info_msg(humble.F2F_CHERRY_PICK_PATH_NOT_EXIST.format(source, src_commit))
                continue
            if not git_path_exists(destination, dest_commit, repo):
                ui_functions.print_info_msg(humble.F2F_CHERRY_PICK_PATH_NOT_EXIST.format(destination, dest_commit))
                continue
            source_excludes = []
            for exclude in folder.source_excludes:
...
def _list_templates():
    ...
    for template in f2f_templates:
        ui_functions.print_info_msg('{}Template {}<-->{}{}'.format(Fore.MAGENTA, template.project1, template.project2, Fore.RESET))
        for folder in template.folders:
            ...
            if len(folder.excludes) > 0:
                ui_functions.print_info_msg("Excludes:")
                for exclude in folder.excludes:
                    ui_functions.print_info_msg('\t{}'.format(exclude.path), header=False)
        ui_functions.print_info_msg('', header=False)
